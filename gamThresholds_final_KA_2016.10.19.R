...
# Script Info #
###############
# PURPOSE: Thresholds from a GAMM or GAM using maximum entropy bootstrapping
# AUTHOR: Scott Large 2013 (revised 2016 by Gavin Fay and Kelly Andrews)
# REVIEWED BY:
# VERSION: 20
...
#########################
#INSERT THE "DATASET" BEING USED BELOW (EITHER "1996" OR "2003" OR an abbreviated name of your dataset - this will be used to decide which indicators and pressures to use, to create results folders and this name will be inserted in the file name of results and figures)

dataset <- 2003 #use either the "1996" or "2003" datasets for the California Current indicators. 1996 dataset includes data from 1996-2014; truncated dataset contains data from 2003-2102 - this is used in comparison with gradient forest dataset

test <- "no"   #use either "yes" or "no". For full data analysis, use "no". If you want to test code with one combination of pressure/response indicators then use "yes". This will trigger ifelse statements to identify "resp" and "dri" used in the full dataset in lines ~83-139 below.

######################
# CHANGES/ ADDITIONS #
######################

# Need to add: Gaussian bootstrap method for GAM; can't seem to get it to work right
...
#setwd("D:\\My Documents\\Manuscripts\\Reference Points\\GAMM results/")
setwd("~/Desktop/codeforreferencepointspaper/")
data.dir <- "data/"
results.dir <- "output/"
if (file.exists(results.dir)==FALSE) dir.create(results.dir)
results.dir <- paste0("output/",dataset,"/")
...
ifelse(dataset==1996,{
        cc <- read.csv("data/1996.2014.reference.points.data.csv")
        ccALL <- cc
        ...
        ##############################################################
        ### Use these response and drivers for the 1996-2014 data ####
        ##############################################################
        
        #### Three relationships (GF_spp_density * NPGO_summer and N_Cop_Anom_winter * NOI_summer) have an error:
        #### Error in lme.formula(y ~ X - 1, random = rand, data = strip.offset(mf),  :   nlminb problem, 
        #### convergence error code = 1   message = singular convergence (7)
        
        ifelse(test=="no",{
          resp <- c(rep("CA sea lion pup growth",16),rep("CA sea lion pup production",16),
                  rep("Groundfish mean trophic level",16), rep("Groundfish species density",16),
                  rep("Groundfish species richness",16), rep("Groundfish Simpson diversity",16),
                  rep("Copepod anomaly summer",16), rep("Copepod anomaly winter",16),
                  rep("Scavenger ratio",16))
          dri <- c(rep(c("Atmospheric pollution","Commercial shipping activity","Dredging",                          
                       "Groundfish landings","Habitat modification","Inorganic pollution",               
                       "Invasive species","Nutrient input","Organic pollution","Total fisheries landings",
                       "NOI summer","NOI winter","NPGO summer","NPGO winter","PDO summer","PDO winter"),9))
                          },  #end "no" statement
          {
          #TO TEST CODE WITH ONE COMBINATION. ONLY USED IF "test" is set to "yes" in Instructions at top of code
          resp <- c(rep("Copepod anomaly winter",1))
          dri <- c(rep(c("Habitat modification"),1))
          #resp <- c(rep("CA sea lion pup production",1))
          #dri  <- c(rep(c("NOI summer"),1))
          }  #end of test code statement
          )          
        },  #end of 1996 variables
       {
        cc <- read.csv("data/2003.2012.reference.points.data.csv")  
        ccALL <- cc
        ccALL$year <- as.numeric(ccALL$year)
        ...
        #############################################################
        ### Use these response and drivers for the 2003-2012 data ####
        ##############################################################
        
        ###WHEN USING THE 2003-2012 DATA AND WHEN ALL COMBINATIONS ARE INCLUDED, TWO RELATIONSHIPS GIVE THE FOLLOWING
        ###ERROR: "Error in lme.formula(y ~ X - 1, random = rand, data = strip.offset(mf),  :nlminb problem, 
        ###convergence error code = 1 message = singular convergence (7)"
        ###THOSE TWO RELATIONSHIPS WERE (N_Cop_Anom_summer * Atmospheric_pollution) and (N_Cop_Anom_summer *
        ###NOI_summer)#####
        
        ifelse(test=="no",{
          resp <- c(rep("CA sea lion pup growth",14),rep("CA sea lion pup production",14),
                  rep("Groundfish mean trophic level",14), rep("Groundfish species density",14),
                  rep("Groundfish species richness",14), rep("Groundfish Simpson diversity",14),
                  rep("Copepod anomaly summer",14), rep("Copepod anomaly winter",14),
                  rep("Scavenger ratio",14))
          dri <- c(rep(c("Atmospheric pollution","Commercial shipping activity","Dredging",                          
                        "Groundfish landings","Habitat modification","Inorganic pollution",               
                        "Invasive species","Total fisheries landings",
                        "NOI summer","NOI winter","NPGO summer","NPGO winter","PDO summer","PDO winter"),9))
                          },  #end test=="no" statement
          {
          #TO TEST CODE WITH ONE COMBINATION, UNCOMMENT THE FOLLOWING TWO LINES:
          resp <- c(rep("CA sea lion pup production",1))
          dri  <- c(rep(c("PDO summer"),1))
          }  #end of test=="yes" part
          ) #end test loop         
        
          }   #end of 2003 variables
       
       )  #end of ifelse for which dataset (1996 or 2003) used


allSummary <- data.frame()
allMagnitude <- data.frame()
allMag.means <- data.frame()

for (imod in 1:length(resp))  #loop analyses over all possible response/driver combinations
  {
    #imod=1
    resp.name <- resp[imod]
    dri.name <- dri[imod]
    ...
    if(length(mydriver)>0 & length(myresponse)>0)
    {
    #
    #####################
    # Fit the GAM model #
    ...
    lme1 <- lme(dev.resid~1,random=~1|rand,correlation=corAR1(form=~ts.length),method='ML')
    lm1 <- lm(dev.resid~1)
    ...
    try(gamm <- gamm(myresponse ~ s(mydriver, bs= "tp",k = ks), optimMmethod="GCV.Cp",
                     se = T,correlation=corAR1(form=~ts.length)))
    if (length(gamm)==0)
    {
      print(imod)
      gamm <- gamm(myresponse ~ s(mydriver),se = T,correlation=corAR1(form=~ts.length))
    }
        #Fit null model to compute deviance explained by gamm
    null  <- glmmPQL(myresponse ~ 1,random=list(rand=~1),family='gaussian')  
    ...
    try(lmac <- gamm(myresponse ~ mydriver, optimMmethod="GCV.Cp",
                     se = T,random=list(rand=~1),correlation=corAR1(form=~ts.length)))
    if (length(lmac)==0) 
    {
      print(imod)
      lmac <- gamm(myresponse ~ mydriver,random=list(rand=~1),se = T,
...
# # Pull out relevant model outputs:
    ...
    colnames(summary.gamm)<- c("MODEL", "RESPONSE", "DRIVER", "AICc", "logLik","dev.expl", "edf", "Pvalue","R-squared","GCV","delta.GCV","p.ac","delta.AIC","diff.dev.expl")
...
allSummary <- rbind(allSummary, summary.gamm, summary.lmac, summary.gam1,
    ...
    } #end if statement for doing the models

} ##imod loop (relevant combinations of driver/responses)

allSummary <- cbind(rownames(allSummary),allSummary)

colnames(allSummary) <- c("PICK#", "MODEL", "RESPONSE", "DRIVER", "AICc", "logLik","dev.expl", "edf", "Pvalue","R-squared","GCV","delta.GCV","p.ac","delta.AIC","diff.dev.expl","best.model")

#Output all relevant model output to data file
bestList <- allSummary
...
if(test=="no")
  {
  write.table(allSummary,paste0(results.dir,paste0(dataset,'.all.models.summary.out')),row.names=FALSE)
  write.csv(allSummary,paste(results.dir,paste0(dataset,".all.models.summary.out.csv")),row.names=F)
  ...
  pick.gamm <- which(allSummary$best.model=="yes" & allSummary$MODEL=="GAMM")
  pick.gam <- which(allSummary$best.model=="yes" & allSummary$MODEL=="GAM")


#############################################################################################################
###CODE FOR MAKING PLOTs FOR RELATIONSHIPS THAT SHOWED GAMM WAS THE BEST MODEL (using "pick.gamm" from above)       
#############################################################################################################

for(row.i in pick.gamm) {    
  ...
  pred <- predict.gam(gamm$gam, newdata = ind.fit, type = "response", se.fit = T)
  ...
  resids <- myresponse-gamm$gam$fitted
  
  # sample multivariate normal for use in gaussian process regression bootstrap
  phi = coef(gamm$lme$modelStruct$corStruct,unconstrained=FALSE)
  ...
  for (i in 1:nb)
  {
    #use next line if just doing residual bootstrap - for "gam" calls
    #respboot[,i] <- gamm$gam$fitted + sample(resids,length(resids),replace=TRUE)
    
    #use next line if doing gaussian process residual bootstrap - for "gamm" calls
    respboot[,i] <- gamm$gam$fitted + resids.use[i,]
    drivboot[,i] <- mydriver
  }
  ...
  #############################
  temp <- diff(gam.data$response,difference=1)
  dif2.line<-diff(temp, difference=1) # Actual 2nd deriv estimate from original smoother
  deriv.matrix.2<-matrix(nrow=sp.len-dif2, ncol=nb) ## create a matrix to for the 2nd deriv estimates
  for(m in 1:nb) {
    derivi<-gam.mat[,m]
    temp <- diff(derivi, difference=1)
    deriv.response<-diff(temp, difference=1)
  ...
  der.plot <- paste0(results.dir, paste0(dataset,".",resp.name, "-", dri.name, ".gamm.png"))
  png(file = der.plot, width = 5, height = 3, units = "in", res = 600)
  ...
  axis(1, dri.ax,cex=1)
  mtext(dri.name, side = 1, line = 1.5, cex=1)
  #
  gam.ax <- pretty(c(CI.mat$GAM[2], CI.mat$GAM[1]), 3)
  axis(2, gam.ax,cex=1)
  mtext(resp.name, side= 2,line=1.5, cex=1)
  #
  # Shading 95% CIs
  ...
  ####################################################################################
  # Will make a plot in your "figures directory"
  der.plot <- paste0(results.dir, paste0(dataset,".",resp.name, "-", dri.name, ".3.panel.gamm.png"))
  ...
  magnitude <- as.data.frame(cbind(resp.name,
                                   dri.name,
  ...
  allMagnitude <- rbind(allMagnitude,magnitude)
  
  resp.name <- NULL
  dri.name <- NULL
}
  
write.csv(allMagnitude,paste0(results.dir,paste0(dataset,'.magnitude of threshold.gamm.csv')),row.names=F)
allMagnitude = NULL
...
for(row.i in pick.gam) {
 ...
 #Use the following code for residual bootstrapping 
  ...
  ####################################################################################
  # Will make a plot in your "figures directory"
  der.plot <- paste0(results.dir, paste0(dataset,".",resp.name, "-", dri.name, ".gam.png"))
  png(file = der.plot, width = 5, height = 3, units = "in", res = 600)
  ...
  ####################################################################################
  # Will make a plot in your "figures directory"
  der.plot <- paste0(results.dir, paste0(dataset,".",resp.name, "-", dri.name, ".3.panel.gam.png"))
  png(file = der.plot, width = 90, height = 130, units = "mm", res = 600)
  layout(matrix(c(1:3), 3,1,  byrow = T),
         widths = lcm(8.45), heights = lcm(c(4.95,3.45, 4.45)),
         respect = F)
...
write.csv(allMagnitude,paste0(results.dir,paste0(dataset,'.magnitude of threshold.gam.csv')),row.names=F)
